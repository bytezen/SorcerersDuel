...
#debug                print "match\n";
        ...
        if ($SpellMatch &&
            !(($sequence eq 'WDDc') && $Info{$Caster}{'ShortLightningUsed'}))
        {
#debug            print "whole match\n";
...
sub GameStatus
{
	...
	if (@Receivers)
	{
	    $Status = "\u$SpellBook $GameType Status for ".
	...
	else
	{
	    ...
	    $Status = "\u$SpellBook $GameType Status (game $GameName, turn $Turn):\n";
