...
sub PrepareForTurn
{
	...
	if ($Info{$wiz}{'CheckNewSpells'})
	{
		...
		if ($Info{$wiz}{'CheckNewSpells'} =~ m/($h)/)
		{
		    @{"${hand}Spells"} = 
			&{"CheckForCast$hand"}($Info{$wiz}{'RH'}, 
					       $Info{$wiz}{'LH'});
		    $newstab = "\L$hand hand" 
		...
		{
		    # Do not check for new choices for this hand
		    @{"${hand}Spells"} = 
			($Info{$wiz}{"${hand}Spell"} eq 'none' ?
			 () : $Info{$wiz}{"${hand}Spell"});
		}
	    }

	    @BothSpells = &CheckForCastBoth($Info{$wiz}{'RH'},
					    $Info{$wiz}{'LH'}, $wiz);
...
#debug              print "SpellEvent:\n$Event";
	...
	else
	{
		...
		@RightSpells = &CheckForCastRight($Info{$wiz}{'RH'},$Info{$wiz}{'LH'});
		@LeftSpells = &CheckForCastLeft($Info{$wiz}{'RH'},$Info{$wiz}{'LH'});
