...
sub DisplayNewGameState
{
    ...
    my @gestures = keys %RemoveSpells;
	 if ($#gestures >= 0)
	 {
		  # Find maximum gesture length to use for formatting
	     my $maxlength = 0;
		  foreach my $g (@gestures)
		  {
		      $maxlength = length($g) if (length($g) > $maxlength);
		  }
	     $disp .= "\n with the following spells removed:";
		  foreach $g (@gestures)
		  {
		      $disp .= "\n   $g";
				$disp .= " " x ($maxlength + 2 - length($g));
				$disp .= join(", ", @{$RemoveSpells{$g}});
		  }
	 }

    @gestures = keys %AddSpells;
	 if ($#gestures >= 0)
	 {
		  # Find maximum gesture length to use for formatting
	     my $maxlength = 0;
	     ...
	     $disp .= "\n with the following spells added:";
		  foreach $g (@gestures)
		  {
		      $disp .= "\n   $g";
				$disp .= " " x ($maxlength + 2 - length($g));
				$disp .= join(", ", @{$AddSpells{$g}});
		  }
	 }
...
sub DisplayChange
{
	...
	$disp .= "   Use the $NewSpellBook Spell Book.\n";
	 ...
	 if ($#gestures >= 0)
	 {
	     foreach $g (@gestures)
		  {
	         $disp .= "   Add the following spell: $g -> " . join (", ", @{$NewAddSpells{$g}}) . "\n";
		  }
	 }
	 ...
	 if ($#gestures >= 0)
	 {
	     foreach $g (@gestures)
		  {
	         $disp .= "   Remove the following spell: $g -> " . join (", ", @{$NewRemoveSpells{$g}}) . "\n";
		  }
	 }
